...
class __tree_iterator { /* __tree_iterator -------------------------------- */
// friend class __rb_tree<Value, Compare, Allocator>;
public:

	typedef bidirectional_iterator_tag	iterator_category;
	// if outside of nesting in tree, change: // ?
	typedef __rb_tree<Value, Compare, Allocator> tree_type;
...
private:
	...
	node_pointer __node_ptr;
	
public:

	__tree_iterator() {}
	__tree_iterator(const node_pointer& x): __node_ptr(x) {}
	__tree_iterator(const __tree_iterator& other)
		: __node_ptr(other.get__node_ptr_np()) {}

	node_pointer get_node_ptr() { return(__node_ptr); }

	__tree_iterator& operator=(const __tree_iterator& other)
		{ __node_ptr = other.__node_ptr; return(*this); }

	reference operator*() const { return(__node_ptr->value); }
	pointer operator->() const { return (std::addressof(operator*())); }
	__tree_iterator& operator++() { this->base = __rb_successor(__node_ptr); return (*this); }
	__tree_iterator& operator--() { this->base = __rb_predecessor(__node_ptr); return (*this); }

	__tree_iterator operator++(int) {
		__tree_iterator old = *this;
		this->base = __rb_successor(__node_ptr);
		return (old);
	}
	
	__tree_iterator operator--(int) {
		__tree_iterator old = *this;
		this->base = __rb_predecessor(__node_ptr);
		return (old);
	}
...
template <class Value, class Compare, class Allocator>
class __tree_const_iterator { /* __tree_const_iterator ------------------- */
// friend class __rb_tree<Value, Compare, Allocator>;
public:
...
private:
	...
	typedef __tree_iterator<Value, Compare, Allocator> 	__non_const_ite;
	node_pointer __node_ptr;

public:

	__tree_const_iterator() {}
	__tree_const_iterator(const node_pointer& x): __node_ptr(x) {}
	...
	reference operator*() const { return(__node_ptr->value); }
	pointer operator->() const { return (std::addressof(operator*())); }
	__tree_const_iterator& operator++() { this->base = __rb_successor(__node_ptr); return (*this); }
	__tree_const_iterator& operator--() { this->base = __rb_predecessor(__node_ptr); return (*this); }

	__tree_const_iterator operator++(int) {
		__tree_const_iterator old = *this;
		this->base = __rb_successor(__node_ptr);
		return (old);
	}
...
*/
template <class Value, class Compare, class Allocator>// = std::allocator<Value> > in map
class __rb_tree {
...
private:
	...
	struct __node { /* __node ----------------------------------------- */
		...
		typedef void (__rb_tree<Value, Compare, Allocator>::__node
			::*const_void_function_pointer) () const;
		typedef void (__rb_tree<Value, Compare, Allocator>::__node
			::*void_function_pointer) () ;

		__node(): p(NULL), left(NULL), right(NULL), isblack(false) { }
		__node(bool isblack): p(NULL), left(NULL), right(NULL), isblack(isblack) { }
	...
	typedef typename allocator_type::template rebind<__node>::other	node_allocator_type;
	// typedef typename std::allocator<__node>	node_allocator_type;

	typedef typename node_allocator_type::pointer						node_pointer;
	typedef typename node_allocator_type::size_type						size_type;
	typedef typename node_allocator_type::difference_type				difference_type;

	// friend class __tree_iterator<Value, Compare, Allocator>; // ?
	typedef __tree_iterator<Value, Compare, Allocator>	iterator;
...
/* ------------------------ construction: --------------------------------- */
	...
	// 	__rb_init();
	// 	_end_node->left = oher._end_node->left;
	// 	_end_node->right = oher._end_node->right;
	// 	_root = oher._end_node->right;
	// 	// ____rb_tree_branching_walk(_root, &__node::__replace_nil); // ?
	// }
...
/* ------------------------ destruction: ---------------------------------- */
	...
	~__rb_tree() {
		____rb_tree_branching_walk(_root, &__node::__destroy);
...
/* ======================== ACCESSORS ===================================== */
	...
	node_pointer leftmost() const { return (rb_min()); }
	node_pointer rightmost() const { return (rb_max()); }
...
/* ======================== QUERIES ======================================= */
	...
	*/
	...
	/*
		PREDECESSOR
		max of the set of values smaller than x, or _nil if that set is empty.
	*/
	node_pointer rb_predecessor(node_pointer x) const {
		...
		node_pointer y = x->p;
		while (y != _nil && y != _end_node && x == y->left) {
			x = y;
			y = y->p;
		}
		return (y);
...
private:
	...
	void __rb_init() {
		...
		// _end_node = _node_allocator.address(__allocate_rb_node(value_type()));
	...
	node_pointer __allocate_rb_node(const_reference value) const {
		...
		_node_allocator.construct(node, __node());
		...
		// node->value = value; // does not compile with const Value arg
		node->value = value_type(value); // does not compile with const Value arg

		// no leaks or double frees with TestClass but A LOT of copies.
...
/* ======================== WALKS ======================================= */
	...
	/*
		...
		Function f is a void const function of class __node.
	*/
	void ____rb_tree_inorder_walk(node_pointer subtree_root,
		typename __node::const_void_function_pointer f) const {
	...
	/*
		...
		Function f is a void non-const function of class __node.
	*/
	void ____rb_tree_branching_walk(node_pointer subtree_root,
		typename __node::void_function_pointer f) {
		if (subtree_root != _nil) {
