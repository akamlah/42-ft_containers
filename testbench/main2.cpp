...
int	main(void)
{
	...
	print_vector(x);
	x.insert((x.begin()) + 1, 1, 73);
	print_vector(x);
	x.insert((x.end()), 1, 432);
	print_vector(x);
	x.insert((x.end()) - 1, 1, 90);
	print_vector(x);
	std::cout << *x.insert(x.end() - 1 , 33) << std::endl;
	std::cout << *x.erase((x.end()) - 1) << std::endl;
	print_vector(x);
	std::cout << *x.erase(x.begin() + 1, x.end() -1 ) << std::endl;
	print_vector(x);
	std::cout << *x.erase(x.begin() + 1, x.end() -1) << std::endl;
	print_vector(x);
	vector<int>	t;
	...
	timePerformance = (timePerformance + (timeEnd.tv_nsec - timeStart.tv_nsec)) * 1e-9;
		...
		std::cout << "FT ";
	#endif
	std::cout << "performance: " <<  timePerformance << " sec.\n";
	#if STD
